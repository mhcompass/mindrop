...
const SECTIONS: Section[] = [
  {
    title: 'ITSM PILLARS',
    mods: [
      m('t_inc', 'Incident Management', 'done', 'done', 'live records engine · list/drawer/transition/assign/worklog · SLA · KB citations', false, { owner: 'ITSM Squad', effort: 'L', critical: true, sensitivity: 'sensitive' }),
      m('t_war', 'Major Incident / War Room', 'done', 'none', 'presentational — FE storyline (clock · roster · comms · actions)', false, { owner: 'ITSM Squad', effort: 'L', critical: true, sensitivity: 'sensitive' }),
      m('t_prob', 'Problem Management', 'done', 'done', 'live records engine · RCA drawer · status lifecycle · agent tools', false, { owner: 'ITSM Squad', effort: 'M', critical: true, sensitivity: 'internal' }),
      m('t_chg', 'Change Management', 'done', 'done', 'live records · change-risk model · CAB approval gate', false, { owner: 'ITSM Squad', effort: 'L', critical: true, sensitivity: 'internal' }),
      m('t_cab', 'CAB Calendar', 'done', 'done', 'live change records · cab_window · conflict flags', false, { owner: 'ITSM Squad', effort: 'M', sensitivity: 'internal' }),
      m('t_cmdb', 'CMDB / Assets', 'done', 'done', 'assets table · /assets + /sites · catalog/drawer (site rollup still seeded)', false, { owner: 'Integrations Squad', effort: 'L', critical: true, sensitivity: 'internal' }),
      m('t_catalog', 'Service Catalog', 'done', 'done', 'catalog_items · /catalog · submit → service_request', false, { owner: 'Experience Squad', effort: 'M', sensitivity: 'internal' }),
      m('t_portal', 'Citizen Portal', 'done', 'done', 'submit → service_request · my-requests live', false, { owner: 'Experience Squad', effort: 'M', channel: true, sensitivity: 'pii' }),
      m('t_oncall', 'On-call Rotation', 'done', 'none', 'presentational — scripted storyline', false, { owner: 'ITSM Squad', effort: 'S', sensitivity: 'pii' }),
      m('t_phone', 'Phone / Telephony', 'done', 'none', 'presentational — scripted demo; no SIP · no server STT', false, { owner: 'Experience Squad', effort: 'L', channel: true, sensitivity: 'pii' }),
  ...
  },
  {
    ...
    mods: [
      m('t_sla', 'SLA Engine', 'partial', 'partial', 'per-priority timers + breach → admin notify + sla_breached event; predictive breach still client-side'),
      m('t_links', 'Cross-links INC↔PRB↔CHG↔CI', 'done', 'done', 'ticket_links table · /links + by-number · link buttons in drawers'),
      ...
      m('t_notif', 'Notifications (round-trip)', 'none', 'partial', 'notifications table live · SLA-breach admin notify wired; portal round-trip to build'),
    ],
  },
  {
    title: 'PLATFORM CORE',
    mods: [
      ...
      m('t_locker', 'My Locker (Case Vault)', 'done', 'done', 'folders · templates · postmortems · EN/AR analysis-language'),
      ...
      m('t_profile', 'Profile / Multi-tenant', 'done', 'done', 'profiles · modules+domains · branding · runtime switch · schema-per-tenant'),
    ],
  },
  {
...
export const MODULE_EDGES: ModuleEdgeDef[] = [
  /* ITIL flow */
  me('me_inc_prob', 't_inc', 't_prob', 'recurrence promotes', { name: 'Pattern Agent', status: 'partial', desc: 'link_records tool connects incident → problem live; automatic recurrence detection still scripted.', tools: ['link_records', 'create_problem'] }),
  me('me_prob_chg', 't_prob', 't_chg', 'permanent fix', { name: 'Change-Risk Agent', status: 'partial', desc: 'Change-risk model live on records + CAB gate; auto-drafted CAB brief still scripted.', tools: ['create_change', 'link_records'] }),
  me('me_chg_cab', 't_chg', 't_cab', 'scheduled at CAB', { name: 'CAB Scheduler', status: 'partial', desc: 'Conflict detection live off real change records (cab_window); window auto-pick still scripted.', tools: ['detect_conflict'] }),
  ...
  me('me_sla_inc', 't_sla', 't_inc', 'breach risk', { name: 'SLA Watcher', status: 'partial', desc: 'Breach → admin notify + sla_breached event live; predictive breach still client-side.', tools: ['notify_breach', 'predict_breach'] }),
  ...
  /* AI involvement */
  me('me_chat_inc', 't_chat', 't_inc', 'triage tools', { name: 'Triage Agent', status: 'ready', desc: 'Classifies + routes via the live records engine — incident tools persist (create/transition/assign).', tools: ['create_incident', 'transition_incident', 'assign_incident'] }),
