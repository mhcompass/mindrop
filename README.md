...
| **Capabilities** | Capability catalogue — one combined list **per domain** (service management · assistant & knowledge · platform & operations · system integrations), each showing what's **available today (✓)** followed by what's **possible within ~2 weeks (+)**, with count chips that double as the key. A presentation-focused view of the same `src/model/plan3.ts` data the Delivery Plan uses |
| **Delivery Plan** | Plain-language delivery plan for **3 engineers, about 2–2.5 weeks** — a checklist of functionality **available today** (grouped, with "final testing" flags on items built in the last few days, plus the system integrations), an at-a-glance summary (team · effort · timeline), and **what this work adds** (grouped by outcome, each item a functionality that can then be enabled, with day estimates). Data in `src/model/plan3.ts` |
